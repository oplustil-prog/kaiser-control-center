...
function collectionRoutesError(error) {
  ...
  console.error("collection_routes.import_batch_detail_failed", { message: error.message });
  return json({ error: "Detail importního batche se teď nepodařilo načíst.", apiStatus: "waiting" }, 500);
}
...
export async function onRequestGet({ request, env, params }) {
  ...
  try {
    const result = await getCollectionImportBatch(env, params.id);
